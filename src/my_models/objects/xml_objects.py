...
class SoftTorsoObject(MujocoXMLObject):
    ...
    def __init__(self, name, damping=None, stiffness=None):
        super().__init__("/home/litao/Documents/robotic-ultrasound-imaging-master-1/src/my_models/assets/objects/soft_human_torso.xml", name=name, duplicate_collision_geoms=False);
...
class SoftBoxObject(MujocoXMLObject):
    ...
    def __init__(self, name, damping=None, stiffness=None):
        super().__init__("/home/litao/Documents/robotic-ultrasound-imaging-master-1/src/my_models/assets/objects/soft_box.xml", name=name, duplicate_collision_geoms=False);
        self.damping = damping
        self.stiffness = stiffness
    ...
    def _get_geom_element(self):
        return self._obj.find("./geom")
    ...
    def set_damping(self, damping):
        ...
        Args:
            damping (float, must be greater than zero): damping parameter to override the ones specified in the XML
        直接在 XML 中覆盖软体阻尼的辅助函数
         参数：
             阻尼（浮点数，必须大于零）：阻尼参数以覆盖 XML 中指定的参数
        """
        assert damping > 0, 'Damping must be greater than zero'
    ...
    def set_stiffness(self, stiffness):
        ...
        Args:
            stiffness (float, must be greater than zero): stiffness parameter to override the ones specified in the XML
            刚度（浮点数，必须大于零）：刚度参数以覆盖 XML 中指定的参数
        """
        assert stiffness > 0, '阻尼必须大于零'
...
class BoxObject(MujocoXMLObject):
    ...
    def __init__(self, name):
        super().__init__("/home/litao/Documents/robotic-ultrasound-imaging-master-1/src/my_models/assets/objects/box.xml", name=name);